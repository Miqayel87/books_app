    ...
    <div class="container">
        ...
        <form action="{{route('book.create')}}" method="POST">
            ...
            <div class="form-group">
                ...
                <input min="1" type="number" name="authors_count" id="authors_count" class="form-control">
            </div>
                        ...
                        <div class="edit_form_container">
                            <div>
                                <form id="editForm" action="{{route('book.update', $book->id)}}" method="POST">
                                    ...
                                    @foreach($book->authors as $author)
                                        <div class="book_author_container" id="{{'authorBook'.$author->id."_".$book->id}}">
                                            {{$author->first_name}}
                                            <input hidden="" value="{{$author->id}}" type="text" name="authors[]">
                                            <button type="button" onclick="deleteBookAuthor('{{'authorBook'.$author->id."_".$book->id}}')" style="background-color: red; height: 100%">x</button>
                                        </div>
                                    @endforeach
